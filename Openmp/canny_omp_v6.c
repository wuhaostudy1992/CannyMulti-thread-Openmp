// multi-thread in blur-x and blur-y and move sum out of blur-x
//derritive_x and derritive_y
//transpose between blur_x and blur_y; and transpose back between derritive_x and derritive_y
//transpose in multi-threads
...
//Edited by Hao Wu for multi-thread 
#define N_T 4
...
void gaussian_smooth(unsigned char *image, int rows, int cols, float sigma,
        short int **smoothedimTranspose);
void make_gaussian_kernel(float sigma, float **kernel, int *windowsize);
void derrivative_x_y(short int *smoothedimTranspose, short int **smoothedim, int rows, int cols,
         ...
         float tlow, float thigh, unsigned char **edge, char *fname)
{
    ...
    short int *smoothedimTranspose,/* The image after gaussian smoothing.      */
              *smoothedim,
    ...
    gaussian_smooth(image, rows, cols, sigma, &smoothedimTranspose);

    /****************************************************************************
     * Compute the first derivative in the x and y directions.
     ****************************************************************************/
    if(VERBOSE) printf("Computing the X and Y first derivatives.\n");
    derrivative_x_y(smoothedimTranspose, &smoothedim, rows, cols, &delta_x, &delta_y);
        ...
        short int **delta_x, short int **delta_y)
{
    ...
    n_thread = N_T;			//Edited by Hao Wu for multi-thread
    omp_set_dynamic(0);			//Edited by Hao Wu for multi-thread
    omp_set_num_threads(n_thread);	//Edited by Hao Wu for multi-thread

    /****************************************************************************
     * Compute the x-derivative. Adjust the derivative at the borders to avoid
     * losing pixels.
     * This is actually y-derivative
     ****************************************************************************/
    #pragma omp parallel private(r, pos)   //Edited by Hao Wu for multi-thread
    {
        if(VERBOSE) printf("   Computing the X-direction derivative.\n");
        #pragma omp for
        for(r=0;r<cols;r++){
            pos = r * rows;
            (*delta_x)[pos] = smoothedimTranspose[pos+1] - smoothedimTranspose[pos];
            pos++;
            for(c=1;c<(cols-1);c++,pos++){
                (*delta_x)[pos] = smoothedimTranspose[pos+1] - smoothedimTranspose[pos-1];
            }
            (*delta_x)[pos] = smoothedimTranspose[pos] - smoothedimTranspose[pos-1];
        }
    }
    ...
    //Transpose edit by Hao Wu
    #pragma omp parallel private(r, c)   //Edited by Hao Wu for multi-thread
    {
        #pragma omp for
        for(r=0;r<rows;r++)
        {
            for(c=0;c<cols;c++)
            {
                *smoothedim[c*rows+r] = smoothedimTranspose[r*cols+c];
            }
        }
    }
    free(smoothedimTranspose);


    /****************************************************************************
     * Compute the y-derivative. Adjust the derivative at the borders to avoid
     * losing pixels.
     * this is actually the x direction
     ****************************************************************************/
    #pragma omp parallel private(c, pos)   //Edited by Hao Wu for multi-thread
    {
        ...
        short int **smoothedimTranspose)
{
    int r, c, rr, cc,     /* Counter variables. */
        ...
        n_thread;          /* number of the threads. */
    float *tempim,        /* Buffer for separable filter gaussian smoothing. */
        *tempimTranpose,  /* Buffer for transposed separable filter gaussian smoothing. */
    ...
    if((tempimTranpose = (float *) calloc(cols*rows, sizeof(float))) == NULL){
        fprintf(stderr, "Error allocating the buffer image.\n");
        exit(1);
    }
    if(((*smoothedimTranspose) = (short int *) calloc(cols*rows,
     ...
     ****************************************************************************/
    n_thread = N_T;			//Edited by Jiang Wan for multi-thread
    omp_set_dynamic(0);			//Edited by Jiang Wan for multi-thread
    omp_set_num_threads(n_thread);	//Edited by Jiang Wan for multi-thread
    #pragma omp parallel private(r, c, cc, dot)   //Edited by Jiang Wan for multi-thread
    {
    ...
    #pragma omp parallel private(r, c)   //Edited by Hao Wu for multi-thread
    {
        ...
        for(r=0;r<rows;r++)
        {
            for(c=0;c<cols;c++)
            {
                tempimTranpose[c*rows+r] = tempim[r*cols+c];
            }
        }
    }
    free(tempim);
    ...
    /****************************************************************************
     * Blur in the y - direction. this matrix is cols * rows
     ****************************************************************************/
    #pragma omp parallel private(r, c, cc, dot)   //Edited by Hao Wu for multi-thread
    {
        ...
        for(r=0;r<cols;r++){
            for(c=0;r<rows;c++){
                ...
                for(cc=(-center);cc<=center;cc++){
                    if(((c+cc) >= 0) && ((c+cc) < cols)){
                        dot += tempimTranpose[(c+cc)*rows+r] * kernel[center+cc];
                    }
                }
                (*smoothedimTranspose)[r*rows+c] = (short int)(dot*BOOSTBLURFACTOR/sum + 0.5);
            }
        }
    }
